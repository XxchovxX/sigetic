// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SIGETIC.Infrastructure.Persistence;

#nullable disable

namespace SIGETIC.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(SigeticDbContext))]
    [Migration("20260827040133_AddAutomaticEquipmentCodes")]
    partial class AddAutomaticEquipmentCodes
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SIGETIC.Domain.Entities.AuditoriaRegistro", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Accion")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("accion");

                    b.Property<string>("DireccionIp")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("direccion_ip");

                    b.Property<string>("Entidad")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("entidad");

                    b.Property<DateTime>("FechaEventoUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_evento_utc");

                    b.Property<string>("MetodoHttp")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("metodo_http");

                    b.Property<string>("Modulo")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("modulo");

                    b.Property<string>("RegistroId")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("registro_id");

                    b.Property<string>("Resumen")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("resumen");

                    b.Property<string>("Rol")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("rol");

                    b.Property<string>("Ruta")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("ruta");

                    b.Property<string>("Usuario")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("usuario");

                    b.HasKey("Id");

                    b.HasIndex("Accion");

                    b.HasIndex("FechaEventoUtc");

                    b.HasIndex("Modulo");

                    b.HasIndex("Usuario");

                    b.ToTable("auditoria_registros", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.BajaEquipo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("DisposicionFinal")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("disposicion_final");

                    b.Property<Guid>("EquipoId")
                        .HasColumnType("uuid")
                        .HasColumnName("equipo_id");

                    b.Property<string>("EstadoFisico")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("estado_fisico");

                    b.Property<DateOnly>("FechaBaja")
                        .HasColumnType("date")
                        .HasColumnName("fecha_baja");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("MotivoBaja")
                        .IsRequired()
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("motivo_baja");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("ResponsableBaja")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("responsable_baja");

                    b.HasKey("Id");

                    b.HasIndex("EquipoId")
                        .IsUnique();

                    b.HasIndex("FechaBaja");

                    b.ToTable("bajas_equipo", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Consumible", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("CodigoInterno")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("codigo_interno");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("color");

                    b.Property<decimal>("CostoUnitario")
                        .HasPrecision(14, 2)
                        .HasColumnType("numeric(14,2)")
                        .HasColumnName("costo_unitario");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("MarcaCompatible")
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("marca_compatible");

                    b.Property<string>("ModelosCompatibles")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("modelos_compatibles");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("Referencia")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("referencia");

                    b.Property<int>("StockActual")
                        .HasColumnType("integer")
                        .HasColumnName("stock_actual");

                    b.Property<int>("StockMinimo")
                        .HasColumnType("integer")
                        .HasColumnName("stock_minimo");

                    b.Property<string>("TipoConsumible")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tipo_consumible");

                    b.Property<string>("UnidadMedida")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("unidad_medida");

                    b.HasKey("Id");

                    b.HasIndex("CodigoInterno")
                        .IsUnique();

                    b.ToTable("consumibles", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Dependencia", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activa")
                        .HasColumnType("boolean")
                        .HasColumnName("activa");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("codigo");

                    b.Property<string>("Correo")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("correo");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre");

                    b.Property<string>("Responsable")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("responsable");

                    b.HasKey("Id");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("dependencias", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1"),
                            Activa = true,
                            Codigo = "PLA",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretaría de Planeación",
                            Responsable = "Secretaría de Planeación"
                        },
                        new
                        {
                            Id = new Guid("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2"),
                            Activa = true,
                            Codigo = "SIS",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Sistemas",
                            Responsable = "Profesional TIC"
                        },
                        new
                        {
                            Id = new Guid("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa3"),
                            Activa = true,
                            Codigo = "SAF",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretaría Administrativa y Financiera",
                            Responsable = "Secretaría Administrativa y Financiera"
                        },
                        new
                        {
                            Id = new Guid("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa4"),
                            Activa = true,
                            Codigo = "SAL",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretaría de Salud y Desarrollo Social",
                            Responsable = "Secretaría de Salud y Desarrollo Social"
                        },
                        new
                        {
                            Id = new Guid("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa5"),
                            Activa = true,
                            Codigo = "GOB",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretaría de Gobierno",
                            Responsable = "Secretaría de Gobierno"
                        });
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Equipo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Almacenamiento")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("almacenamiento");

                    b.Property<string>("CodigoInterno")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("codigo_interno");

                    b.Property<string>("Dependencia")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("dependencia");

                    b.Property<string>("DireccionIp")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("direccion_ip");

                    b.Property<string>("DireccionMac")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("direccion_mac");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("estado");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateOnly>("FechaIngreso")
                        .HasColumnType("date")
                        .HasColumnName("fecha_ingreso");

                    b.Property<string>("FuncionarioAsignado")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("funcionario_asignado");

                    b.Property<string>("Marca")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("marca");

                    b.Property<string>("MemoriaRam")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("memoria_ram");

                    b.Property<string>("Modelo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("modelo");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("observaciones");

                    b.Property<string>("Procesador")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("procesador");

                    b.Property<string>("Serial")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("serial");

                    b.Property<string>("SistemaOperativo")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("sistema_operativo");

                    b.Property<string>("TipoEquipo")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo_equipo");

                    b.Property<string>("UbicacionFisica")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("ubicacion_fisica");

                    b.HasKey("Id");

                    b.HasIndex("CodigoInterno")
                        .IsUnique();

                    b.HasIndex("Serial")
                        .IsUnique();

                    b.ToTable("equipos", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCurso", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("Categoria")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("categoria");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(1200)
                        .HasColumnType("character varying(1200)")
                        .HasColumnName("descripcion");

                    b.Property<string>("DirigidoA")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("dirigido_a");

                    b.Property<int>("DuracionMinutos")
                        .HasColumnType("integer")
                        .HasColumnName("duracion_minutos");

                    b.Property<string>("EntidadCertificadora")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasDefaultValue("Secretaría de Planeación")
                        .HasColumnName("entidad_certificadora");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<int>("PuntajeMinimo")
                        .HasColumnType("integer")
                        .HasColumnName("puntaje_minimo");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("titulo");

                    b.HasKey("Id");

                    b.HasIndex("Activo");

                    b.HasIndex("Categoria");

                    b.ToTable("formacion_cursos", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCursoDependencia", b =>
                {
                    b.Property<Guid>("CursoId")
                        .HasColumnType("uuid")
                        .HasColumnName("curso_id");

                    b.Property<Guid>("DependenciaId")
                        .HasColumnType("uuid")
                        .HasColumnName("dependencia_id");

                    b.HasKey("CursoId", "DependenciaId");

                    b.HasIndex("DependenciaId");

                    b.ToTable("formacion_cursos_dependencias", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCursoUsuario", b =>
                {
                    b.Property<Guid>("CursoId")
                        .HasColumnType("uuid")
                        .HasColumnName("curso_id");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.HasKey("CursoId", "UsuarioId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("formacion_cursos_usuarios", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionIntento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Aprobado")
                        .HasColumnType("boolean")
                        .HasColumnName("aprobado");

                    b.Property<string>("CodigoCertificado")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("codigo_certificado");

                    b.Property<Guid>("CursoId")
                        .HasColumnType("uuid")
                        .HasColumnName("curso_id");

                    b.Property<DateTime>("FechaPresentacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_presentacion_utc");

                    b.Property<string>("ParticipanteCorreo")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("participante_correo");

                    b.Property<string>("ParticipanteNombre")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("participante_nombre");

                    b.Property<int>("Puntaje")
                        .HasColumnType("integer")
                        .HasColumnName("puntaje");

                    b.Property<int>("RespuestasCorrectas")
                        .HasColumnType("integer")
                        .HasColumnName("respuestas_correctas");

                    b.Property<int>("TotalPreguntas")
                        .HasColumnType("integer")
                        .HasColumnName("total_preguntas");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasIndex("CodigoCertificado")
                        .IsUnique();

                    b.HasIndex("CursoId");

                    b.HasIndex("UsuarioId");

                    b.ToTable("formacion_intentos", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionMaterial", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CursoId")
                        .HasColumnType("uuid")
                        .HasColumnName("curso_id");

                    b.Property<int>("Orden")
                        .HasColumnType("integer")
                        .HasColumnName("orden");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("titulo");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(800)
                        .HasColumnType("character varying(800)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("CursoId");

                    b.ToTable("formacion_materiales", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionOpcion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("EsCorrecta")
                        .HasColumnType("boolean")
                        .HasColumnName("es_correcta");

                    b.Property<int>("Orden")
                        .HasColumnType("integer")
                        .HasColumnName("orden");

                    b.Property<Guid>("PreguntaId")
                        .HasColumnType("uuid")
                        .HasColumnName("pregunta_id");

                    b.Property<string>("Texto")
                        .IsRequired()
                        .HasMaxLength(600)
                        .HasColumnType("character varying(600)")
                        .HasColumnName("texto");

                    b.Property<string>("TextoRelacionado")
                        .HasMaxLength(600)
                        .HasColumnType("character varying(600)")
                        .HasColumnName("texto_relacionado");

                    b.HasKey("Id");

                    b.HasIndex("PreguntaId");

                    b.ToTable("formacion_opciones", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionPregunta", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CursoId")
                        .HasColumnType("uuid")
                        .HasColumnName("curso_id");

                    b.Property<string>("Explicacion")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("explicacion");

                    b.Property<int>("Orden")
                        .HasColumnType("integer")
                        .HasColumnName("orden");

                    b.Property<string>("Texto")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("texto");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasDefaultValue("SeleccionUnica")
                        .HasColumnName("tipo");

                    b.HasKey("Id");

                    b.HasIndex("CursoId");

                    b.ToTable("formacion_preguntas", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionRespuesta", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool?>("Correcta")
                        .HasColumnType("boolean")
                        .HasColumnName("correcta");

                    b.Property<string>("DatosRespuesta")
                        .HasColumnType("text")
                        .HasColumnName("datos_respuesta");

                    b.Property<Guid>("IntentoId")
                        .HasColumnType("uuid")
                        .HasColumnName("intento_id");

                    b.Property<Guid?>("OpcionId")
                        .HasColumnType("uuid")
                        .HasColumnName("opcion_id");

                    b.Property<Guid>("PreguntaId")
                        .HasColumnType("uuid")
                        .HasColumnName("pregunta_id");

                    b.Property<string>("RespuestaTexto")
                        .HasMaxLength(4000)
                        .HasColumnType("character varying(4000)")
                        .HasColumnName("respuesta_texto");

                    b.HasKey("Id");

                    b.HasIndex("IntentoId");

                    b.HasIndex("OpcionId");

                    b.HasIndex("PreguntaId");

                    b.ToTable("formacion_respuestas", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Funcionario", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("Cargo")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("cargo");

                    b.Property<string>("Correo")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("correo");

                    b.Property<Guid>("DependenciaId")
                        .HasColumnType("uuid")
                        .HasColumnName("dependencia_id");

                    b.Property<string>("Documento")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("documento");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("NombreCompleto")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre_completo");

                    b.Property<string>("Telefono")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("telefono");

                    b.Property<string>("TipoVinculacion")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasDefaultValue("Funcionario")
                        .HasColumnName("tipo_vinculacion");

                    b.HasKey("Id");

                    b.HasIndex("DependenciaId");

                    b.HasIndex("Documento")
                        .IsUnique();

                    b.ToTable("funcionarios", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.HistorialConsumibleImpresora", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("Cantidad")
                        .HasColumnType("integer")
                        .HasColumnName("cantidad");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("color");

                    b.Property<int?>("ContadorPaginas")
                        .HasColumnType("integer")
                        .HasColumnName("contador_paginas");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateOnly>("FechaMovimiento")
                        .HasColumnType("date")
                        .HasColumnName("fecha_movimiento");

                    b.Property<Guid>("ImpresoraId")
                        .HasColumnType("uuid")
                        .HasColumnName("impresora_id");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("Referencia")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("referencia");

                    b.Property<string>("ResponsableEntrega")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("responsable_entrega");

                    b.Property<string>("TipoConsumible")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tipo_consumible");

                    b.Property<string>("TipoMovimiento")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tipo_movimiento");

                    b.HasKey("Id");

                    b.HasIndex("FechaMovimiento");

                    b.HasIndex("ImpresoraId");

                    b.ToTable("historial_consumibles_impresora", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Impresora", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CodigoInterno")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("codigo_interno");

                    b.Property<Guid>("DependenciaId")
                        .HasColumnType("uuid")
                        .HasColumnName("dependencia_id");

                    b.Property<string>("DireccionIp")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("direccion_ip");

                    b.Property<string>("DireccionMac")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("direccion_mac");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("estado");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateOnly>("FechaIngreso")
                        .HasColumnType("date")
                        .HasColumnName("fecha_ingreso");

                    b.Property<Guid?>("FuncionarioAsignadoId")
                        .HasColumnType("uuid")
                        .HasColumnName("funcionario_asignado_id");

                    b.Property<string>("Marca")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("marca");

                    b.Property<string>("Modelo")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("modelo");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("Serial")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("serial");

                    b.Property<string>("TecnologiaImpresion")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tecnologia_impresion");

                    b.Property<string>("TipoImpresora")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tipo_impresora");

                    b.Property<string>("UbicacionFisica")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("ubicacion_fisica");

                    b.HasKey("Id");

                    b.HasIndex("CodigoInterno")
                        .IsUnique();

                    b.HasIndex("DependenciaId");

                    b.HasIndex("FuncionarioAsignadoId");

                    b.HasIndex("Serial")
                        .IsUnique();

                    b.ToTable("impresoras", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.InventarioDeteccion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("DatosJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("datos_json");

                    b.Property<string>("DireccionIpOrigen")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("direccion_ip_origen");

                    b.Property<string>("Estado")
                        .IsConcurrencyToken()
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("estado");

                    b.Property<DateTime>("ExpiraUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expira_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateTime?>("FechaRecepcionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_recepcion_utc");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("token_hash");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasIndex("ExpiraUtc");

                    b.HasIndex("TokenHash")
                        .IsUnique();

                    b.HasIndex("UsuarioId", "FechaCreacionUtc");

                    b.ToTable("inventario_detecciones", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MantenimientoEquipo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ActividadesRealizadas")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("actividades_realizadas");

                    b.Property<string>("Diagnostico")
                        .IsRequired()
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("diagnostico");

                    b.Property<string>("DocumentoRecibe")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("documento_recibe");

                    b.Property<Guid>("EquipoId")
                        .HasColumnType("uuid")
                        .HasColumnName("equipo_id");

                    b.Property<string>("EstadoResultante")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("estado_resultante");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateTime?>("FechaFirmaUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_firma_utc");

                    b.Property<DateOnly>("FechaMantenimiento")
                        .HasColumnType("date")
                        .HasColumnName("fecha_mantenimiento");

                    b.Property<string>("FirmaRecibe")
                        .HasColumnType("text")
                        .HasColumnName("firma_recibe");

                    b.Property<string>("FirmaTecnico")
                        .HasColumnType("text")
                        .HasColumnName("firma_tecnico");

                    b.Property<string>("NombreRecibe")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre_recibe");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<DateOnly?>("ProximaRevision")
                        .HasColumnType("date")
                        .HasColumnName("proxima_revision");

                    b.Property<string>("RepuestosUtilizados")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("repuestos_utilizados");

                    b.Property<string>("TecnicoResponsable")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("tecnico_responsable");

                    b.Property<string>("TipoMantenimiento")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo_mantenimiento");

                    b.HasKey("Id");

                    b.HasIndex("EquipoId");

                    b.HasIndex("FechaMantenimiento");

                    b.ToTable("mantenimientos_equipo", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MantenimientoImpresora", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ActividadesRealizadas")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("actividades_realizadas");

                    b.Property<int?>("ContadorPaginas")
                        .HasColumnType("integer")
                        .HasColumnName("contador_paginas");

                    b.Property<string>("Diagnostico")
                        .IsRequired()
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("diagnostico");

                    b.Property<string>("DocumentoRecibe")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("documento_recibe");

                    b.Property<string>("EstadoResultante")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("estado_resultante");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateTime?>("FechaFirmaUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_firma_utc");

                    b.Property<DateOnly>("FechaMantenimiento")
                        .HasColumnType("date")
                        .HasColumnName("fecha_mantenimiento");

                    b.Property<string>("FirmaRecibe")
                        .HasColumnType("text")
                        .HasColumnName("firma_recibe");

                    b.Property<string>("FirmaTecnico")
                        .HasColumnType("text")
                        .HasColumnName("firma_tecnico");

                    b.Property<Guid>("ImpresoraId")
                        .HasColumnType("uuid")
                        .HasColumnName("impresora_id");

                    b.Property<string>("NombreRecibe")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre_recibe");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<DateOnly?>("ProximaRevision")
                        .HasColumnType("date")
                        .HasColumnName("proxima_revision");

                    b.Property<string>("RepuestosUtilizados")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("repuestos_utilizados");

                    b.Property<string>("TecnicoResponsable")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("tecnico_responsable");

                    b.Property<string>("TipoMantenimiento")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("tipo_mantenimiento");

                    b.HasKey("Id");

                    b.HasIndex("FechaMantenimiento");

                    b.HasIndex("ImpresoraId");

                    b.ToTable("mantenimientos_impresora", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MovimientoConsumible", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("Cantidad")
                        .HasColumnType("integer")
                        .HasColumnName("cantidad");

                    b.Property<Guid>("ConsumibleId")
                        .HasColumnType("uuid")
                        .HasColumnName("consumible_id");

                    b.Property<decimal>("CostoTotal")
                        .HasPrecision(14, 2)
                        .HasColumnType("numeric(14,2)")
                        .HasColumnName("costo_total");

                    b.Property<decimal>("CostoUnitario")
                        .HasPrecision(14, 2)
                        .HasColumnType("numeric(14,2)")
                        .HasColumnName("costo_unitario");

                    b.Property<Guid?>("DependenciaId")
                        .HasColumnType("uuid")
                        .HasColumnName("dependencia_id");

                    b.Property<string>("Destino")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("destino");

                    b.Property<string>("DocumentoSoporte")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("documento_soporte");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateOnly>("FechaMovimiento")
                        .HasColumnType("date")
                        .HasColumnName("fecha_movimiento");

                    b.Property<Guid?>("ImpresoraId")
                        .HasColumnType("uuid")
                        .HasColumnName("impresora_id");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("Responsable")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("responsable");

                    b.Property<int>("StockResultante")
                        .HasColumnType("integer")
                        .HasColumnName("stock_resultante");

                    b.Property<string>("TipoMovimiento")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("tipo_movimiento");

                    b.HasKey("Id");

                    b.HasIndex("ConsumibleId");

                    b.HasIndex("DependenciaId");

                    b.HasIndex("FechaMovimiento");

                    b.HasIndex("ImpresoraId");

                    b.ToTable("movimientos_consumibles", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Permiso", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Accion")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("accion");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("codigo");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("descripcion");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("Modulo")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("modulo");

                    b.HasKey("Id");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.ToTable("permisos", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.ProgramacionMantenimiento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CodigoActivo")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("codigo_activo");

                    b.Property<string>("CorreoTecnico")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("correo_tecnico");

                    b.Property<Guid?>("EquipoId")
                        .HasColumnType("uuid")
                        .HasColumnName("equipo_id");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("estado");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateOnly>("FechaProgramada")
                        .HasColumnType("date")
                        .HasColumnName("fecha_programada");

                    b.Property<string>("Frecuencia")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("frecuencia");

                    b.Property<TimeOnly?>("HoraProgramada")
                        .HasColumnType("time without time zone")
                        .HasColumnName("hora_programada");

                    b.Property<Guid?>("ImpresoraId")
                        .HasColumnType("uuid")
                        .HasColumnName("impresora_id");

                    b.Property<string>("NombreActivo")
                        .IsRequired()
                        .HasMaxLength(220)
                        .HasColumnType("character varying(220)")
                        .HasColumnName("nombre_activo");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1500)
                        .HasColumnType("character varying(1500)")
                        .HasColumnName("observaciones");

                    b.Property<string>("TecnicoResponsable")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("tecnico_responsable");

                    b.Property<string>("TipoActivo")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("tipo_activo");

                    b.Property<string>("TipoMantenimiento")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo_mantenimiento");

                    b.Property<DateTime?>("UltimaNotificacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ultima_notificacion_utc");

                    b.HasKey("Id");

                    b.HasIndex("CorreoTecnico");

                    b.HasIndex("EquipoId");

                    b.HasIndex("Estado");

                    b.HasIndex("FechaProgramada");

                    b.HasIndex("ImpresoraId");

                    b.HasIndex("TipoActivo");

                    b.ToTable("programaciones_mantenimiento", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Rol", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("descripcion");

                    b.Property<bool>("EsSistema")
                        .HasColumnType("boolean")
                        .HasColumnName("es_sistema");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("nombre");

                    b.HasKey("Id");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("roles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("11111111-1111-1111-1111-111111111111"),
                            Activo = true,
                            Descripcion = "Acceso total al sistema SIGETIC.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Administrador"
                        },
                        new
                        {
                            Id = new Guid("22222222-2222-2222-2222-222222222222"),
                            Activo = true,
                            Descripcion = "Gestión técnica de inventario, mantenimientos, impresoras, consumibles y mesa de ayuda.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Administrador TIC"
                        },
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333333"),
                            Activo = true,
                            Descripcion = "Apoyo operativo en mantenimientos, tickets, impresoras y consumibles.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Auxiliar de Sistemas"
                        },
                        new
                        {
                            Id = new Guid("44444444-4444-4444-4444-444444444444"),
                            Activo = true,
                            Descripcion = "Usuario interno para creación y consulta de solicitudes.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Funcionario"
                        },
                        new
                        {
                            Id = new Guid("55555555-5555-5555-5555-555555555555"),
                            Activo = true,
                            Descripcion = "Acceso de solo consulta a reportes y trazabilidad.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Consulta / Control Interno"
                        },
                        new
                        {
                            Id = new Guid("66666666-6666-6666-6666-666666666666"),
                            Activo = true,
                            Descripcion = "Apoyo administrativo de la Secretaría Administrativa Financiera para consumibles, reportes y trazabilidad.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Auxiliar Administrativo SAF"
                        },
                        new
                        {
                            Id = new Guid("77777777-7777-7777-7777-777777777777"),
                            Activo = true,
                            Descripcion = "Acceso directivo de consulta, reportes, analítica, tickets y actualización de consumibles.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretario Administrativo Financiero"
                        },
                        new
                        {
                            Id = new Guid("88888888-8888-8888-8888-888888888888"),
                            Activo = true,
                            Descripcion = "Acceso para visualizar dashboard y crear solicitudes en mesa de ayuda.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Secretario de Despacho"
                        },
                        new
                        {
                            Id = new Guid("99999999-9999-9999-9999-999999999910"),
                            Activo = true,
                            Descripcion = "Ejecucion operativa de soportes, mantenimientos, inventario TIC, impresoras, consumibles y programaciones preventivas.",
                            EsSistema = true,
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Nombre = "Tecnico TIC"
                        });
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.RolPermiso", b =>
                {
                    b.Property<Guid>("RolId")
                        .HasColumnType("uuid")
                        .HasColumnName("rol_id");

                    b.Property<Guid>("PermisoId")
                        .HasColumnType("uuid")
                        .HasColumnName("permiso_id");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.HasKey("RolId", "PermisoId");

                    b.HasIndex("PermisoId");

                    b.ToTable("roles_permisos", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.SecuenciaCodigoEquipo", b =>
                {
                    b.Property<string>("Clave")
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("clave");

                    b.Property<DateTime>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<int>("UltimoNumero")
                        .HasColumnType("integer")
                        .HasColumnName("ultimo_numero");

                    b.HasKey("Clave");

                    b.ToTable("secuencias_codigo_equipo", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.TicketMesaAyuda", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int?>("CalificacionAmabilidad")
                        .HasColumnType("integer")
                        .HasColumnName("calificacion_amabilidad");

                    b.Property<int?>("CalificacionAtencion")
                        .HasColumnType("integer")
                        .HasColumnName("calificacion_atencion");

                    b.Property<int?>("CalificacionSolucion")
                        .HasColumnType("integer")
                        .HasColumnName("calificacion_solucion");

                    b.Property<int?>("CalificacionTiempo")
                        .HasColumnType("integer")
                        .HasColumnName("calificacion_tiempo");

                    b.Property<string>("Categoria")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("categoria");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("codigo");

                    b.Property<string>("ComentarioSatisfaccion")
                        .HasMaxLength(1200)
                        .HasColumnType("character varying(1200)")
                        .HasColumnName("comentario_satisfaccion");

                    b.Property<string>("Dependencia")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("dependencia");

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("descripcion");

                    b.Property<bool>("Eliminado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("eliminado");

                    b.Property<string>("EliminadoPor")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("eliminado_por");

                    b.Property<string>("EquipoCodigo")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("equipo_codigo");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("estado");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateOnly?>("FechaCompromiso")
                        .HasColumnType("date")
                        .HasColumnName("fecha_compromiso");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<DateTime?>("FechaEliminacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_eliminacion_utc");

                    b.Property<DateTime?>("FechaEncuestaUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_encuesta_utc");

                    b.Property<DateOnly>("FechaSolicitud")
                        .HasColumnType("date")
                        .HasColumnName("fecha_solicitud");

                    b.Property<string>("ImpresoraCodigo")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("impresora_codigo");

                    b.Property<string>("Prioridad")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("prioridad");

                    b.Property<string>("ResponsableAsignado")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("responsable_asignado");

                    b.Property<string>("Solicitante")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("solicitante");

                    b.Property<string>("Solucion")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("solucion");

                    b.Property<Guid?>("UsuarioSolicitanteId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_solicitante_id");

                    b.HasKey("Id");

                    b.HasIndex("Codigo")
                        .IsUnique();

                    b.HasIndex("Eliminado");

                    b.HasIndex("Estado");

                    b.HasIndex("FechaSolicitud");

                    b.HasIndex("Prioridad");

                    b.HasIndex("UsuarioSolicitanteId");

                    b.ToTable("tickets_mesa_ayuda", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.TicketMesaAyudaHistorial", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Detalle")
                        .HasMaxLength(1200)
                        .HasColumnType("character varying(1200)")
                        .HasColumnName("detalle");

                    b.Property<string>("EstadoAnterior")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("estado_anterior");

                    b.Property<string>("EstadoNuevo")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("estado_nuevo");

                    b.Property<DateTime>("FechaEventoUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_evento_utc");

                    b.Property<Guid>("TicketId")
                        .HasColumnType("uuid")
                        .HasColumnName("ticket_id");

                    b.Property<string>("TipoEvento")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo_evento");

                    b.Property<string>("Usuario")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("usuario");

                    b.HasKey("Id");

                    b.HasIndex("FechaEventoUtc");

                    b.HasIndex("TicketId");

                    b.ToTable("tickets_mesa_ayuda_historial", (string)null);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Usuario", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("boolean")
                        .HasColumnName("activo");

                    b.Property<string>("Cargo")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("cargo");

                    b.Property<string>("Correo")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("correo");

                    b.Property<Guid?>("DependenciaId")
                        .HasColumnType("uuid")
                        .HasColumnName("dependencia_id");

                    b.Property<DateTime?>("FechaActualizacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_actualizacion_utc");

                    b.Property<DateTime>("FechaCreacionUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("fecha_creacion_utc");

                    b.Property<bool>("GestionFormacionHabilitada")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("gestion_formacion_habilitada");

                    b.Property<DateTime?>("GestionFormacionHastaUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("gestion_formacion_hasta_utc");

                    b.Property<string>("GoogleSubject")
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("google_subject");

                    b.Property<string>("NombreCompleto")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("character varying(180)")
                        .HasColumnName("nombre_completo");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("password_hash");

                    b.Property<Guid>("RolId")
                        .HasColumnType("uuid")
                        .HasColumnName("rol_id");

                    b.Property<string>("TipoVinculacion")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("tipo_vinculacion");

                    b.Property<DateTime?>("UltimoAccesoUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ultimo_acceso_utc");

                    b.HasKey("Id");

                    b.HasIndex("Correo")
                        .IsUnique();

                    b.HasIndex("DependenciaId");

                    b.HasIndex("GoogleSubject")
                        .IsUnique();

                    b.HasIndex("RolId");

                    b.ToTable("usuarios", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("99999999-9999-9999-9999-999999999999"),
                            Activo = true,
                            Correo = "admin@sigetic.local",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            NombreCompleto = "Administrador SIGETIC",
                            PasswordHash = "PBKDF2$100000$U0lHRVRJQy1TRUVELTEyMw==$lSmKzh6GuxzgqXW1/AaCDXUBki5UtsLLy4RB3VaXeJU=",
                            RolId = new Guid("11111111-1111-1111-1111-111111111111")
                        },
                        new
                        {
                            Id = new Guid("99999999-9999-9999-9999-999999999901"),
                            Activo = true,
                            Correo = "secretario.administrativo@sigetic.local",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            NombreCompleto = "Secretario Administrativo Financiero",
                            PasswordHash = "PBKDF2$100000$U0lHRVRJQy1TRUVELTEyMw==$i1+AHOAmtLD32rOHnRdrVxH3XvFNTiAnBIOfrXk6aDc=",
                            RolId = new Guid("77777777-7777-7777-7777-777777777777")
                        },
                        new
                        {
                            Id = new Guid("99999999-9999-9999-9999-999999999902"),
                            Activo = true,
                            Correo = "administrador.tic@sigetic.local",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            NombreCompleto = "Administrador TIC",
                            PasswordHash = "PBKDF2$100000$U0lHRVRJQy1TRUVELTEyMw==$KhPXRkL0wtv+wc3FCc0wToQSYLxy2UgpWh3bhfFOJNk=",
                            RolId = new Guid("22222222-2222-2222-2222-222222222222")
                        },
                        new
                        {
                            Id = new Guid("99999999-9999-9999-9999-999999999903"),
                            Activo = true,
                            Correo = "auxiliar.administrativo@sigetic.local",
                            FechaCreacionUtc = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            NombreCompleto = "Auxiliar Administrativo - Secretaría Administrativa Financiera",
                            PasswordHash = "PBKDF2$100000$U0lHRVRJQy1TRUVELTEyMw==$4aCPuTJlP/3ueUt3g/rNa54vbAj9p8b05xhGsUHw5Uw=",
                            RolId = new Guid("66666666-6666-6666-6666-666666666666")
                        });
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.BajaEquipo", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Equipo", "Equipo")
                        .WithMany()
                        .HasForeignKey("EquipoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Equipo");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCursoDependencia", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionCurso", "Curso")
                        .WithMany("DependenciasDestino")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.Dependencia", "Dependencia")
                        .WithMany()
                        .HasForeignKey("DependenciaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("Dependencia");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCursoUsuario", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionCurso", "Curso")
                        .WithMany("UsuariosDestino")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionIntento", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionCurso", "Curso")
                        .WithMany("Intentos")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Curso");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionMaterial", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionCurso", "Curso")
                        .WithMany("Materiales")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionOpcion", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionPregunta", "Pregunta")
                        .WithMany("Opciones")
                        .HasForeignKey("PreguntaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Pregunta");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionPregunta", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionCurso", "Curso")
                        .WithMany("Preguntas")
                        .HasForeignKey("CursoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Curso");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionRespuesta", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.FormacionIntento", "Intento")
                        .WithMany("Respuestas")
                        .HasForeignKey("IntentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.FormacionOpcion", "Opcion")
                        .WithMany()
                        .HasForeignKey("OpcionId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("SIGETIC.Domain.Entities.FormacionPregunta", "Pregunta")
                        .WithMany()
                        .HasForeignKey("PreguntaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Intento");

                    b.Navigation("Opcion");

                    b.Navigation("Pregunta");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Funcionario", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Dependencia", "Dependencia")
                        .WithMany()
                        .HasForeignKey("DependenciaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Dependencia");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.HistorialConsumibleImpresora", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Impresora", "Impresora")
                        .WithMany()
                        .HasForeignKey("ImpresoraId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Impresora");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Impresora", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Dependencia", "Dependencia")
                        .WithMany()
                        .HasForeignKey("DependenciaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.Funcionario", "FuncionarioAsignado")
                        .WithMany()
                        .HasForeignKey("FuncionarioAsignadoId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Dependencia");

                    b.Navigation("FuncionarioAsignado");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.InventarioDeteccion", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MantenimientoEquipo", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Equipo", "Equipo")
                        .WithMany()
                        .HasForeignKey("EquipoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Equipo");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MantenimientoImpresora", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Impresora", "Impresora")
                        .WithMany()
                        .HasForeignKey("ImpresoraId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Impresora");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.MovimientoConsumible", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Consumible", "Consumible")
                        .WithMany()
                        .HasForeignKey("ConsumibleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.Dependencia", "Dependencia")
                        .WithMany()
                        .HasForeignKey("DependenciaId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("SIGETIC.Domain.Entities.Impresora", "Impresora")
                        .WithMany()
                        .HasForeignKey("ImpresoraId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Consumible");

                    b.Navigation("Dependencia");

                    b.Navigation("Impresora");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.ProgramacionMantenimiento", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Equipo", "Equipo")
                        .WithMany()
                        .HasForeignKey("EquipoId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("SIGETIC.Domain.Entities.Impresora", "Impresora")
                        .WithMany()
                        .HasForeignKey("ImpresoraId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Equipo");

                    b.Navigation("Impresora");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.RolPermiso", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Permiso", "Permiso")
                        .WithMany()
                        .HasForeignKey("PermisoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SIGETIC.Domain.Entities.Rol", "Rol")
                        .WithMany()
                        .HasForeignKey("RolId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Permiso");

                    b.Navigation("Rol");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.TicketMesaAyuda", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Usuario", null)
                        .WithMany()
                        .HasForeignKey("UsuarioSolicitanteId")
                        .OnDelete(DeleteBehavior.SetNull);
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.TicketMesaAyudaHistorial", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.TicketMesaAyuda", "Ticket")
                        .WithMany("Historial")
                        .HasForeignKey("TicketId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Ticket");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.Usuario", b =>
                {
                    b.HasOne("SIGETIC.Domain.Entities.Dependencia", "Dependencia")
                        .WithMany()
                        .HasForeignKey("DependenciaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("SIGETIC.Domain.Entities.Rol", "Rol")
                        .WithMany()
                        .HasForeignKey("RolId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Dependencia");

                    b.Navigation("Rol");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionCurso", b =>
                {
                    b.Navigation("DependenciasDestino");

                    b.Navigation("Intentos");

                    b.Navigation("Materiales");

                    b.Navigation("Preguntas");

                    b.Navigation("UsuariosDestino");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionIntento", b =>
                {
                    b.Navigation("Respuestas");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.FormacionPregunta", b =>
                {
                    b.Navigation("Opciones");
                });

            modelBuilder.Entity("SIGETIC.Domain.Entities.TicketMesaAyuda", b =>
                {
                    b.Navigation("Historial");
                });
#pragma warning restore 612, 618
        }
    }
}
